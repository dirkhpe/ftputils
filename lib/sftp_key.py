"""
sftp Handler - connection is created with key pair.
"""
...
class SftpHandler:
    ...
    def __init__(self, **props):
        ...
        :param props: Connection dictionary, containing host, user and pwd or pkf (private key file)
        """
        # Todo: implement connection with pwd
        ...
        if self.cnopts.hostkeys.lookup(host) is None:
            self.get_hostkeys(**props)
        else:
            # At this point the hostkeys are available for the host.
            self.sftp = pysftp.Connection(host, username=user, private_key=pkf, cnopts=self.cnopts)
        return

    def get_hostkeys(self, **props):
        """
        ...
        self.sftp = pysftp.Connection(host, username=user, private_key=pkf, cnopts=self.cnopts)
        logging.info("Connected to new host {}, caching its hostkey".format(host))
        hostkeys.add(host, self.sftp.remote_server_key.get_name(), self.sftp.remote_server_key)
